...
double ILS_ES(VecInt *Solucion, const MatDouble *distancias, int m) {
    ...
    int iter_ini = 0;
    fitnessActual = ES_inte(&solucionAMejorar, &noSeleccionados, distancias, &iter_ini, iter_fin);
    ...
    while(iteraciones != iteracionesMax) {
        ...
        iter_ini = 0;
        double nuevo_fitness = ES_inte(&nuevaSolucion, &nuevoNoSeleccionados, distancias, &iter_ini, iter_fin);
